...
cbuffer PerCamera : register(b1)
{
	float4x4  ViewProj					: packoffset(c0.x); [ViewProjection]
		...
		float     Exposure : packoffset(c15.z); [Exposure]
...
float3 IsotropicLobe(const ShadingParams shading, const PixelParams pixel, const Light light, const float3 h, float NoV, float NoL, float NoH, float LoH)
{
...
float3 SpecularLobe(const ShadingParams shading, const PixelParams pixel, const Light light, const float3 h, float NoV, float NoL, float NoH, float LoH)
{
	//#if defined(MATERIAL_HAS_ANISOTROPY)
	//    return anisotropicLobe(pixel, light, h, NoV, NoL, NoH, LoH);
	//#else
	return IsotropicLobe(shading, pixel, light, h, NoV, NoL, NoH, LoH);
	//#endif
}
...
float3 SurfaceShading(const ShadingParams shading, const PixelParams pixel, const Light light, float occlusion)
{
	...
	float3 Fr = SpecularLobe(shading, pixel, light, h, NoV, NoL, NoH, LoH);
